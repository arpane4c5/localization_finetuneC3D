...
# -*- coding: utf-8 -*-
"""
Created on Mon April 01 01:34:25 2019
@author: Arpan
@Description: Utils file to extract frames from folder videos and dump to disk.
...
deep network.
Total Execution Time for 78 Train vids : 381.479 secs (njobs=5, batch=5)
Total Execution Time for 34 Val vids :  708.930 secs (njobs=1, batch=1)
Total Execution Time for 30 Test vids : 539.557 secs (njobs=1, batch=1)
"""
...
def extract_vid_frames(srcFolderPath, destFolderPath, njobs=1, batch=10, stop='all'):
    ...
    ------
    srcFolderPath: str
        path to folder which contains the subfolder with videos
    destFolderPath: str
...
#                print "Done "+str(traversed_tot+traversed)+" : "+sf+"/"+vid
    ...
    for i in range(int(nrows/batch)):
        ...
        for j in range(batch):
            if batch_diffs[j] is not None:
                # save the .npy files to destination
    ...
    if last_batch_size > 0:
        ...
        for j in range(last_batch_size):
            if batch_diffs[j] is not None:
                # save the .npy files to the destination
...
if __name__=='__main__':
    ...
    destPath = "/home/arpan/VisionWorkspace/Cricket/dataset_25_fps_test_set_112x112"
    if not os.path.exists(srcPath):
        srcPath = "/home/arpan/DATA_Drive/Cricket/dataset_25_fps_test_set"
        destPath = "/home/arpan/DATA_Drive/Cricket/dataset_25_fps_test_set_112x112"
